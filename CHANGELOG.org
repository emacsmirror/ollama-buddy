...
** <2026-07-15 Wed> *7.6.1*

Fix =void-function ollama-buddy--open-chat= when the chat is opened from the role transient menu before the main package is loaded.

*** Bug Fixes

- *Role transient menu did not load the main package* -- =ollama-buddy-role-transient-menu= is autoloaded from =ollama-buddy-transient= (not =ollama-buddy=), so a =use-package :bind= setup that only binds that command never loads =ollama-buddy.el=.  The transient module forward-declares =ollama-buddy--open-chat= via =declare-function=, which satisfies the byte-compiler but does not define the function at run time, so selecting "Open Chat" signalled =Symbol's function definition is void: ollama-buddy--open-chat=.  =ollama-buddy-role-transient-menu= now calls =(require 'ollama-buddy)= before building the menu, matching the existing =ollama-buddy-transient-menu-wrapper= pattern.  Reported in [[https://github.com/captainflasmr/ollama-buddy/issues/24][#24]].

** <2026-06-22 Mon> *7.6.0*

Dynamic cloud model discovery — the static =ollama-buddy-cloud-models= list is replaced with automatic fetching from ollama.com.

*** Features

- *Dynamic cloud model discovery* -- =ollama-buddy-cloud-models= is now populated automatically from the =https://ollama.com/api/tags= endpoint on first access (model picker, Model Management buffer, provider summary), so newly released cloud models appear without a package update.  No static fallback list is maintained: cloud models require internet access, so a hardcoded offline list added no value.  The fetch is session-cached (one attempt per Emacs session) and skipped in airplane mode.  =ollama-buddy-cloud-models-url= (a defcustom) controls the endpoint.
- *=/sync= command* -- New slash command and =M-x ollama-buddy-cloud-sync-models= force-refresh the cloud model catalog on demand.  Also bound to =y= in the auth transient menu.
- *=?* transient binding* -- Pressing =?= in the chat buffer now opens the transient menu, matching the convention used by other Emacs packages.  =C-c O= remains as an alternative.

*** Breaking Changes

- * =ollama-buddy-cloud-models= is now a defvar, not a defcustom* -- The variable is populated dynamically and no longer appears in the Customize interface.  Users who customised the old defcustom should remove the saved value; the dynamic fetch supersedes it.

** <2026-06-22 Mon> *7.5.5*

Preserve newlines in streamed responses after a thinking block.

*** Bug Fixes

- *Reasoning newline stripping* -- Fixed a bug where the leading-newline trim applied after a thinking/reasoning block could stay active for the rest of the response, swallowing every subsequent newline and collapsing paragraphs and lists into one unbroken run of text.  This was most visible with models that stream newlines as their own chunks combined with markdown-to-org conversion (e.g. gemma).  Leading blank lines immediately after the thinking block are still trimmed, but once real response text begins newlines are now left intact.  Thanks to Shintaro KATAYAMA (=@shintaro-katayama=) for reporting and diagnosing this in [[https://github.com/captainflasmr/ollama-buddy/pull/23][#23]].

** <2026-06-22 Mon> *7.5.4*

OpenCode Go dynamic model discovery and chat-endpoint routing fix.

*** Features

- *OpenCode Go model discovery* -- =ollama-buddy-opencode.el= now fetches its model list dynamically from the Go subscription's OpenAI-style listing endpoint (=ollama-buddy-opencode-models-endpoint=, https://opencode.ai/zen/go/v1/models) instead of a hand-maintained list, so newly released models (e.g. =minimax-m3=, =glm-5.2=, =qwen3.7-max=, =kimi-k2.7-code=, =hy3-preview=) appear automatically.  The static =ollama-buddy-opencode-chat-models= list remains as an offline fallback when discovery is unavailable.

*** Bug Fixes

- *OpenCode Go "Empty input messages" error* -- All OpenCode Go models are OpenAI-native, but DeepSeek V4 and MiniMax M2.x were previously routed through the Anthropic-compatible =/messages= endpoint, which mis-proxies them so the upstream reports =Empty input messages=.  Every OpenCode Go model now routes through =/chat/completions= by default.  The Anthropic shape is retained as an opt-in override via the (now empty by default) =ollama-buddy-opencode-msg-models= and =ollama-buddy-opencode-msg-model-families= options.

*** Maintenance

- *Packaging conventions* -- Declared the =transient= dependency in the main package header, shortened the package summary line, added =ollama-buddy-annotate.el=, =ollama-buddy-opencode.el=, and =ollama-buddy-plan.el= to the build/CI file list, added a missing =cl-lib= require, removed a dead Emacs-28 =org-fold= compatibility shim, replaced an unnecessary =with-eval-after-load= for savehist with a direct registration, and dropped ineffective per-file =Package-Requires= / =Version= headers from secondary files (a multi-file package is built from the main file's header).

** <2026-05-19 Tue> *7.5.3*

Skill export command for Claude Code / opencode integration.

*** Features

- *Skill export* -- New =ollama-buddy-user-prompts-export-skills= and =ollama-buddy-user-prompts-export-skill= commands convert user prompts to the =SKILL.md= format used by Claude Code, opencode, and compatible AI agent tools.  Each prompt becomes a named skill directory under =~/.claude/skills/= (configurable via =ollama-buddy-skills-export-directory=) with YAML frontmatter description, trigger phrases, and markdown body content.  A secondary target directory (=ollama-buddy-skills-export-agents-directory=) is supported for project-local =.agents/skills/= copies.

** <2026-04-30 Thu> *7.5.2*

OpenCode Go usage tracking and Model Management reorganization.

*** Features

- *OpenCode Go usage tracking* -- Added =ollama-buddy-opencode-usage-url= and =ollama-buddy-opencode-session-token= to =ollama-buddy-opencode.el=.  When configured, it parses your unique workspace Go page to display rolling, weekly, and monthly usage percentages alongside Ollama Cloud stats.
- *Usage section in Model Management* -- Reorganized the =*Ollama Models Management*= buffer (=C-c M=) to include a dedicated =* Usage= section at the top.  This section consolidates budget/usage pie charts for both Ollama Cloud and OpenCode Go, keeping them clearly separated from model selection.
- *Cloud cookie lifecycle note* -- Updated documentation to note that both Ollama and OpenCode session cookies can rotate upon re-authentication, requiring a fresh copy from browser DevTools if usage stats stop appearing.
